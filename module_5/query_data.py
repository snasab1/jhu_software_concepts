...
def total_applicants_fall_2024():
    """Calculate the total number of applicants for Fall 2024."""
...
def average_gpa_and_gre():
    """Calculate average GPA and GRE scores of all applicants."""
...
def acceptance_percentage_fall_2024():
    """Calculate the acceptance percentage for Fall 2024 applicants."""
...
def average_gpa_accepted_fall_2024():
    """Calculate the average GPA of Accepted applicants for Fall 2024."""
    query = f"""
        SELECT AVG(gpa) FROM {TABLE_NAME}
        WHERE term = %s 
        AND status LIKE %s
...
def jhu_applicants_masters_cs():
    """Count the number of Johns Hopkins applicants for a Masters in Computer Science."""
    query = f"""
